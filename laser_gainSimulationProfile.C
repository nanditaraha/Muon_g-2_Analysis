...
void gainSimulationTest(int cycle,int fill, int laser) {
  ...
  */
  ...
  TProfile *Phist2 = new TProfile("Phist2"," ...",bins,0,700,0.95,1.0);
  //TProfile *PLaser2 = new TProfile("PLaser2"," ...",bins,0,700,0.95,1.0);
  ...
  for(int c=0;c<cycle;c++){
	...
	//PLaser2->Fill(decayTimes[i],f1->Eval(decayTimes[i]));
	laser_on=true;
      }      	
	...
	Phist2->Fill(decayTimes_noLaser[i],f1->Eval(decayTimes_noLaser[i]));
      }
  ...
  hf = new TH1D("Laser","Gain versus time - Laser Shots",7e2,0,700);
